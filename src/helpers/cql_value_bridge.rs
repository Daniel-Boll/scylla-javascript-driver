use napi::bindgen_prelude::{BigInt, Either11, Either12};
use scylla::frame::response::result::CqlValue;

use std::collections::HashMap;

use crate::types::{
  decimal::Decimal, duration::Duration, float::Float, list::List, uuid::Uuid, varint::Varint,
};
...
macro_rules! define_expected_type {
    ($lifetime:lifetime, $($t:ty),+) => {
      pub type ParameterNativeTypes<$lifetime> = Either11<$($t),+>;
      pub type ParameterWithMapType<$lifetime> = Either12<$($t),+, HashMap<String, ParameterNativeTypes<$lifetime>>>;
      pub type JSQueryParameters<$lifetime> = napi::Result<Vec<HashMap<String, ParameterWithMapType<$lifetime>>>>;
    };
}

define_expected_type!('a, u32, String, &'a Uuid, BigInt, &'a Duration, &'a Decimal, bool, Vec<u32>, &'a Float, &'a Varint, &'a List);
...
impl<'a> ToCqlValue for ParameterWithMapType<'a> {
  fn to_cql_value(&self) -> CqlValue {
    match self {
      ...
      ParameterWithMapType::L(map) => CqlValue::UserDefinedType {
